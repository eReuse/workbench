...
def get_user_input():
    ...
    if settings.getboolean('DEFAULT', 'comment'):
        value = input("Additional comments (empty to skip): ").strip()
        if value:
...
def main(argv=None):
    ...
    if settings.get('DEFAULT', 'FLASK') or settings.get('DEFAULT', 'BROKER'):
        push_json(settings, {
            '_uuid': test_uuid4, 'created': datetime.datetime.utcnow(), 'device': {'_uuid': device_uuid4},
            'filename': filename, 'localpath': localpath, 'signed_data': signed_data
        })
    ...
    if settings.get('DEFAULT', 'FLASK') or settings.get('DEFAULT', 'BROKER'):
        data = {
            '_uuid': test_uuid4, 'device': {'_uuid': device_uuid4}, 'created': datetime.datetime.utcnow(),
            'inventory': args.inventory, 'copy_to_usb': settings.getboolean('DEFAULT', 'copy_to_usb')
        }
        push_json(settings, data)
    ...
    if settings.get('DEFAULT', 'FLASK') or settings.get('DEFAULT', 'BROKER'):
        data = {
            '_uuid': test_uuid4, 'device': {'_uuid': device_uuid4}, 'created': datetime.datetime.utcnow(),
            'image_name': None, 'install_image_ok': None
        }
        if image_name:
