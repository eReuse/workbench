...
apt-get install -y dmidecode fsarchiver gnupg lshw python-dmidecode smartmontools stress usbmount python-gnupg python-lxml python-paramiko
...
# Autostart
echo "clear ; sudo device-inventory ; di-stress-test ; sudo di-install-image" >> /home/ubuntu/.profile
...
# A) Ubuntu
#sudo mkisofs -D -r -V "$IMAGE_NAME" -cache-inodes -J -l -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -o ../eReuseOS-7.1a4.iso .

# B) Debian
sudo genisoimage -D -r -V "eReuseOS" -cache-inodes -J -l -b isolinux/isolinux.bin -c isolinux/boot.cat -no-emul-boot -boot-load-size 4 -boot-info-table -o ../eReuseOS-7.1a4.iso .
